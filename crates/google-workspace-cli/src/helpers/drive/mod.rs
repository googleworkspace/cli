...
///
/// If `output` is provided, it is used as-is. Otherwise the `default_name`
...
pub(super) fn resolve_output_path(
    ...
    match output {
        Some(p) => {
            ...
            // If the output path looks like a directory (ends with separator), append the default name
        ...
        None => Ok(std::path::PathBuf::from(default_name)),
    }
}
