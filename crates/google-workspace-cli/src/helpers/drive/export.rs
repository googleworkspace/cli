...
pub(super) async fn handle_export(matches: &ArgMatches) -> Result<(), GwsError> {
    ...
    // Build output filename: strip any existing extension and add the export format extension
    let dest = match output_path {
        Some(p) => resolve_output_path(Some(p.as_str()), remote_name)?,
        None => {
            let stem = Path::new(remote_name)
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or(remote_name);
            std::path::PathBuf::from(format!("{}.{}", stem, format))
        }
    };
