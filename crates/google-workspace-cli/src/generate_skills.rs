...
mod tests {
    ...
    fn test_shared_skill_frontmatter_uses_block_sequences() {
        ...
        assert!(
            content.contains("npm install -g @googleworkspace/cli"),
            "shared skill should document CLI installation"
        );
        assert!(
            content.contains("gws auth setup --login"),
            "shared skill should document first-time authentication setup"
        );
        assert!(
            content.contains("https://www.googleapis.com/auth/chat.spaces.readonly"),
            "shared skill should document explicit Chat scopes"
        );
    }
